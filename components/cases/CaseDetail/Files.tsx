...
import { Upload, Trash2, Download, FileText, ImageIcon, File, Loader2, ExternalLink } from 'lucide-react'
...
export function CaseFilesSection({ caseId, isEditable }: Props) {
  ...
  const fetchSignedUrl = useCallback(async (fileId: string): Promise<string | null> => {
    ...
    try {
      ...
      if (!res.ok) { toast.error('URLの取得に失敗しました'); return null }
      const { url } = await res.json()
      setFiles(prev => prev.map(f => f.id === fileId ? { ...f, signedUrl: url, urlLoading: false } : f))
      return url
    } catch {
  ...
  const uploadFiles = async (fileList: FileList, forceType?: FileType) => {
    ...
    for (const file of Array.from(fileList)) {
      if (file.size > MAX_FILE_SIZE_MB * 1024 * 1024) {
        errors.push(`${file.name}: ${MAX_FILE_SIZE_MB}MB を超えています`); continue
      }
    ...
    <div className="space-y-4">
      ...
      <section className="rounded-lg border border-border bg-card overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          {isEditable && (
            <div
              ref={dropRef}
              className={cn('flex cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-border py-7 text-sm text-muted-foreground transition-colors hover:border-primary/50 hover:bg-primary/5', uploading && 'pointer-events-none opacity-50')}
              ...
              onDrop={e => { e.preventDefault(); dropRef.current?.classList.remove('border-primary', 'bg-primary/5'); if (e.dataTransfer.files.length) uploadFiles(e.dataTransfer.files) }}
            >
              <input ref={inputRef} type="file" multiple className="hidden" onChange={e => e.target.files && uploadFiles(e.target.files)} />
              {uploading ? <><Loader2 className="h-5 w-5 animate-spin" /><span>アップロード中...</span></> : <><Upload className="h-5 w-5" /><span>クリックまたはドラッグ＆ドロップ</span><span className="text-xs">PDF・画像・Excel・Word など（最大 {MAX_FILE_SIZE_MB}MB）</span></>}
            </div>
          )}
          {otherFiles.length === 0
      ...
      <section className="rounded-lg border border-border bg-card overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          {isEditable && (
            <>
              <input ref={layoutInputRef} type="file" multiple accept="image/*,application/pdf" className="hidden"
                onChange={e => e.target.files && uploadFiles(e.target.files, 'レイアウト図')} />
              <button onClick={() => layoutInputRef.current?.click()} disabled={uploading}
      ...
      {previewFile && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={() => setPreviewFile(null)}>
          <div className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-xl bg-card shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-border px-5 py-3">
              <p className="text-sm font-medium truncate">{previewFile.label || previewFile.file_name}</p>
              <div className="flex shrink-0 items-center gap-2 ml-4">
                <button onClick={() => handleDownload(previewFile)} className="inline-flex items-center gap-1 rounded-md border border-input px-3 py-1 text-xs hover:bg-muted"><Download className="h-3.5 w-3.5" />DL</button>
                {previewFile.signedUrl && <a href={previewFile.signedUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-md border border-input px-3 py-1 text-xs hover:bg-muted"><ExternalLink className="h-3.5 w-3.5" />新しいタブ</a>}
                <button onClick={() => setPreviewFile(null)} className="rounded-md border border-input px-3 py-1 text-xs hover:bg-muted">✕</button>
              </div>
            </div>
...
function LayoutCard({ file, isEditable, onPreview, onLabelChange, onDelete, onFetchUrl }: {
  file: FileWithUrl; isEditable: boolean
  onPreview: () => void; onLabelChange: (l: string) => void
  onDelete: () => void; onFetchUrl: () => Promise<string | null>
}) {
