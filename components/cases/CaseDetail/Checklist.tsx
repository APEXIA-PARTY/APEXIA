          ...
          <div className="space-y-1.5">
            ...
            {items.filter(i => i.state === '確認中').map(item => (
              <div key={item.id} className="flex items-center gap-2 group">
                ...
                {isEditable ? (
                  <textarea
                    className="flex-1 rounded border-0 bg-transparent px-1 py-0.5 text-sm text-muted-foreground focus:bg-muted/50 focus:outline-none focus:ring-1 focus:ring-ring resize-none leading-snug"
                    rows={1}
          ...
          <div className="space-y-1.5">
            ...
            {items.filter(i => i.state === '確定').map(item => (
              <div key={item.id} className="flex items-center gap-2 group">
                ...
                {isEditable ? (
                  <textarea
                    className="flex-1 rounded border-0 bg-transparent px-1 py-0.5 text-sm focus:bg-muted/50 focus:outline-none focus:ring-1 focus:ring-ring resize-none leading-snug"
                    rows={1}
                    ref={el => { if (el) autoResize(el) }}
                    defaultValue={item.item}
                    onInput={e => autoResize(e.currentTarget)}
          ...
          <div className="space-y-1.5">
            ...
            {items.filter(i => i.state === 'キャンセル').map(item => (
              <div key={item.id} className="flex items-center gap-2 group opacity-70">
                ...
                {isEditable ? (
                  <textarea
                    className="flex-1 rounded border-0 bg-transparent px-1 py-0.5 text-sm line-through text-muted-foreground focus:no-underline focus:bg-muted/50 focus:outline-none focus:ring-1 focus:ring-ring resize-none leading-snug"
                    rows={1}
                    ref={el => { if (el) autoResize(el) }}
                    defaultValue={item.item}
                    onInput={e => autoResize(e.currentTarget)}
