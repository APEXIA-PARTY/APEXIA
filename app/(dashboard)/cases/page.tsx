...
export default async function CasesPage({
  ...
  if (searchParams.status) {
    query = query.eq('status', searchParams.status as CaseStatus)
  }

  if (searchParams.media_id) {
    query = query.eq('media_id', searchParams.media_id)
  }

  if (searchParams.floor_id) {
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-border bg-card p-4">
        <form method="GET" action="/cases" className="flex flex-wrap items-end gap-3">
          ...
          <Link href="/cases" className="py-2 text-sm text-muted-foreground hover:underline">
            クリア
          </Link>
        </form>
      </div>
