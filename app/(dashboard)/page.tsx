...
import { formatDate, formatCurrency, formatCurrencyShort, formatDateTime } from '@/lib/utils/format'
...
import {
  ...
  DollarSign, BarChart2, AlertTriangle, Calendar
} from 'lucide-react'
...
export default async function DashboardPage() {
  ...
  // 直近の確定・仮押さえ
  const { data: upcomingCasesRaw } = await supabase
    .from('cases')
  ...
  const upcomingCases = Array.isArray(upcomingCasesRaw) ? upcomingCasesRaw : []
  ...
  const recentCases = Array.isArray(recentCasesRaw) ? recentCasesRaw : []

  // ランキング：認知経路
  const { data: mediaListRaw } = await supabase
    .from('media_master')
    .select('id,name')
    .eq('is_active', true)
  ...
  const categoryList = Array.isArray(categoryListRaw) ? categoryListRaw : []

  const categoryRank = categoryList
    .map((c: any) => ({
      id: c.id,
      name: c.name,
      revenue: rows
        .filter((r) => r.event_category_id === c.id && REVENUE_STATUSES.includes(r.status))
        .reduce((s, r) => s + (r.estimate_amount ?? 0), 0),
    }))
    .sort((a, b) => b.revenue - a.revenue)
    .slice(0, 3)
